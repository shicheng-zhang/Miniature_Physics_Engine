...
static inline void frame_timer_init (frame_timer *timer_object) {
    ...
    timer_object -> delta_time = 0.016;
    timer_object -> maximum_delta_time = 0.05; //Maximum Transfer Rate at 50 ms
} static inline void frame_timer_update (frame_timer *timer_object) {
    gint64 current_monotonic_time = g_get_monotonic_time ();
    float elapsed_seconds = (float) (current_monotonic_time - timer_object -> last_iteration_time) / 1000000.0;
    ...
    if (elapsed_seconds <= 0.0) {elapsed_seconds = 0.016;}
