...
int main_algorithm (int argc, char *argv []) {
    ...
    gtk_widget_add_events (main_window_container, GDK_KEY_PRESS_MASK | GDK_KEY_RELEASE_MASK | GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);
    //Signalling
    ...
    g_signal_connect (main_window_container, "button-press-event", G_CALLBACK (on_button_press), &main_inputs);
    g_signal_connect (main_window_container, "button-release-event", G_CALLBACK (on_button_release), &main_inputs);
    //Add Objects
